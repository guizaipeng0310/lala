...
function TreeNode(text, url, iconOpen, iconOpenHover, iconClosed, iconClosedHover) {
	...
	this.ROOT = this.PATH + "root.gif";
	this.CHILDREN = this.PATH + "children.gif";
	...
	this.node.appendChild(this.ldiv);
	//this.node.appendChild(this.nodeIcon);
	this.ldiv.appendChild(this.label);
	...
	this.label.style.lineHeight = "25px";
	...
	if (url) {
	    ...
	    this.label.target = "mainIframe";
	    this.ldiv.style.backgroundImage = "url(" + this.CHILDREN + ")";
	}else {
		this.label.style.color = "white";
		this.ldiv.style.backgroundImage = "url(" + this.ROOT + ")";
	}
